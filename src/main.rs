...
struct App {
    repo: Box<dyn Repository<library_entry::LibraryEntry>>
}
...
mod CommandApi {
    use crate::{
        ...
        fill_repo_with_dummy_data, library_entry,
        repository::{LibraryEntriesRepository, Repository},
    };

    #[get("/?<author>&<read>")]
    pub fn list(author: Option<String>, read: Option<bool>) {
        ...
        let mut command =
            CommandFactory::new_command(CommandType::ADD, repo.as_mut(), Box::new(args));

        command.execute();
    }

    #[get("/?<title>&<author>")]
    pub fn add(title: Option<String>, author: Option<String>) {
        let args = ArgumentsProviderRequest::new();
        let mut repo: Box<dyn Repository<library_entry::LibraryEntry>> =
            Box::new(LibraryEntriesRepository::new());
        fill_repo_with_dummy_data(&mut repo);
        let mut command =
            CommandFactory::new_command(CommandType::LIST, repo.as_mut(), Box::new(args));
...
fn rocket() -> _ {
    ...
    let _rocket = rocket::build()
        .mount("/list", routes![CommandApi::list])
        .mount("/add", routes![CommandApi::add])
        .manage(state);
    _rocket
}
